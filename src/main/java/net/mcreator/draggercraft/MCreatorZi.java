...
public class MCreatorZi extends Elementsdraggercraft.ModElement {
	@ObjectHolder("draggercraft:zi")
	public static final ModDimension dimension = null;
	@ObjectHolder("draggercraft:zi")
	public static final Item block = null;
	@ObjectHolder("draggercraft:zi_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;

	public MCreatorZi(Elementsdraggercraft instance) {
		super(instance, 11);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("zi"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("draggercraft:zi")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("draggercraft:zi"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("draggercraft:zi"));
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ModTrigger().setRegistryName("zi"));
	}

	public static class ModTrigger extends Item {
		public ModTrigger() {
			super(new Item.Properties().group(MCreatorDraggerStuff.tab).maxDamage(64));
		}
		...
		public void animateTick(BlockState state, World world, BlockPos pos, Random random) {
			...
			if (random.nextInt(110) == 0)
				world.playSound(pos.getX() + 0.5, pos.getY() + 0.5, pos.getZ() + 0.5,
						(net.minecraft.util.SoundEvent) ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation(("block.lava.ambient"))),
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| this.world.getBlockState(blockpos.down()).getBlock() != MCreatorZi_Portal_Block.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == MCreatorZi_Portal_Block.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != MCreatorZi_Portal_Block.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != MCreatorZi_Portal_Block.block
								...
								for (int l7 = i2; l7 < i2 + 2; ++l7) {
									...
									for (int i10 = 0; i10 < 4; ++i10) {
										for (int k10 = -1; k10 < 4; ++k10) {
											...
											if (k10 < 0 && !this.world.getBlockState(blockpos$mutableblockpos).getMaterial().isSolid() || k10 >= 0
													&& !this.world.isAirBlock(blockpos$mutableblockpos)) {
												continue label214;
											}
										}
				...
				for (int j7 = -1; j7 <= 1; ++j7) {
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos, flag ? MCreatorZi_Portal_Block.block.getDefaultState().getBlock()
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, MCreatorZi_Portal_Block.block.getDefaultState().getBlock()
								.getDefaultState(), 3);
					}
				}
			}
	...
	public static class CustomDimension extends Dimension {
		...
		public Vec3d getFogColor(float cangle, float ticks) {
			return new Vec3d(0.364705882353, 0.262745098039, 0.262745098039);
		}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return MCreatorZiEarth.block.getDefaultState();
				}
